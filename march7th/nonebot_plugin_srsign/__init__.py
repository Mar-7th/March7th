...
async def _(bot: Bot, event: Event):
    ...
    sr_sign_info = await mys_api.call_mihoyo_sign(cookie=cookie, role_uid=sr_uid)
    ...
    if isinstance(sr_sign_info, dict):
        ...
        if retcode == 0:
            if is_risk:
                sr_sign_info, error_message = await geetest_handle(
                    sign_data=sr_sign_info, cookie=cookie, role_uid=sr_uid
                )
...
async def geetest_handle(sign_data: dict, cookie: str, role_uid: str = "0"):
    ...
    page_url = (
        "https://webstatic.mihoyo.com/bbs/event/signin/hkrpg/e202304121516551.html"
    )
    gt = sign_data["data"]["gt"]
    challenge = sign_data["data"]["challenge"]
    captcha_result, error_message = await geetest_get_result(
        api_key=captcha_api_key,
        api_endpoint=captcha_endpoint,
        gt=gt,
        challenge=challenge,
        page_url=page_url,
    )
    if error_message == "":
        return 0, await sign_with_geetest(
            geetest_challenge=captcha_result["challenge"],
...
async def geetest_get_result(
    api_key: str, api_endpoint: str, gt: str, challenge: str, page_url: str
):
    raise NotImplementedError("请联系管理员开启验证码绕过")


async def sign_with_geetest(
    ...
    if (
        geetest_challenge == ""
        or geetest_challenge is None
        or geetest_seccode == ""
        or geetest_seccode is None
        or geetest_validate == ""
        or geetest_validate is None
    ):
    ...
    return await mys_api.call_mihoyo_sign(
        cookie=cookie, role_uid=role_uid, extra_headers=extra_headers
    )
